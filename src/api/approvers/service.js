...
async function approveAtmosphere(request) {
    ...
    if (!user) throw 'service:approveAtmosphere: Missing required property'
    ...
    if (
        ...
        user.occupation.name.toLowerCase().indexOf('student') >= 0
    ) {
        logger.info(
            `approveAtmosphere: Pend student user "${user.username}" for request ${request.id}`
        )
        await request.pend()
        await sendAtmosphereSignupMessage(
            request,
            `${intro}
...
For more information please see the FAQ: ${faqUrl}`
    ...
    logger.info(
        `approveAtmosphere: Deny user "${user.username}" for request ${request.id}`
    )
    await request.deny()
    await sendAtmosphereSignupMessage(
        request,
...
function getAtmosphereConversationBody(questions, answers) {
    ...
    if (questions && questions.length > 0) {
        ...
        for (question of questions) {
            ...
            else if (question.type == 'text') body += answer.value_text
...
async function sendAtmosphereSignupMessage(request, responseMessage) {
    ...
    const body = getAtmosphereConversationBody(
        service.questions,
        request.answers
    )
    ...
    if (intercom) {
        ...
        await intercom.assignConversation(
            conversation.id,
            process.env.INTERCOM_ADMIN_TIER1_ATMOSPHERE_ID
        )
    }
}
