...
export async function getServerSideProps({ req }) {
  try {
    ...
    if (forms && typeof forms === 'object' && forms.forms && Array.isArray(forms.forms)) {
      console.warn('API returned forms wrapped in object, using forms.forms')
      return { props: { forms: forms.forms } }
    }

    // Fallback for any other case
    console.warn('Unexpected forms API response format:', typeof forms, forms)
    return { props: { forms: [] } }
