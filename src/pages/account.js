...
for (const r of properties.regions) {
    if (!(r.country_id in regions)) regions[r.country_id] = []
...
const Account = () => {
    ...
    const title = (
        <div style={{ display: 'flex', alignItems: 'center' }}>
            <PersonIcon fontSize="large" style={{ marginRight: '0.25em' }} />
            Account
        </div>
    )
    ...
    return (
        ...
        >
            <Container maxWidth="md">
                {forms &&
                    forms
                        .filter(f => !reviewMode || !f.hideReviewMode)
                        .map((form, index) => (
                            <Box
                                key={index}
...
const EmailForm = ({ emails, title, subtitle, onChange }) => {
    ...
    const validateEmail = async value => {
        if (!isEmail(value)) return 'A valid email address is required'
    ...
    const statusMsg = email =>
        email.verified ? (
            ...
            <>
                A confirmation email has been sent. Click on the link in the
                email to verify that this is your address.
                <br />
                {email.sent ? (
                    '[ Sent! ]'
                ) : (
                    <Link onClick={() => resendConfirmationEmail(email)}>
    ...
    return (
        <div>
            ...
            <AddEmailAddressDialog
                ...
                handleSubmit={
                    !isEmpty(emailToAdd) && !validationError
                        ? submitEmail
                        : null
                }
            />
