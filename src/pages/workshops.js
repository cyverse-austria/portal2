...
const Workshops = props => {
    ...
    const otherWorkshops = workshops.filter(
        w => !userWorkshops.find(w2 => w2.id == w.id)
    )

    const timeNow = Date.now()
    const mine = userWorkshops.filter(
        w => new Date(w.enrollment_ends).getTime() > timeNow
    )
    const past = userWorkshops.filter(
        w => new Date(w.enrollment_ends).getTime() <= timeNow
    )
    const upcoming = otherWorkshops.filter(
        w => w.is_public && new Date(w.enrollment_ends).getTime() > timeNow
    )
...
const MyWorkshops = ({ workshops }) => {
    const content =
        workshops.length > 0 ? (
            ...
            <Typography variant="body1">
                Looks like you aren't attending any workshops. If you enroll in
                one, you'll find it here.
            </Typography>
        )
    ...
    return (
        <div>
            ...
            <br />
            <Typography variant="body1">
                Request to host your own workshop{' '}
                <Link href="requests/8">here</Link>.
            </Typography>
        </div>
    )
}
...
const UpcomingWorkshops = ({ workshops }) => {
    if (workshops.length > 0) return <WorkshopGrid workshops={workshops} />
...
const PastWorkshops = ({ workshops }) => {
    ...
    return (
        <Typography variant="body1">
            Looks like you haven't attended any workshops.
        </Typography>
    )
}
                    ...
                    <>
                        ...
                        >
                            {isHost ? (
                                ...
                                <b>You are a workshop organizer</b>
                            ) : null}
                        </span>
                        {!workshop.is_public && (
                            <Button
...
export async function getServerSideProps({ req }) {
    try {
        ...
        if (
            workshops &&
            typeof workshops === 'object' &&
            workshops.workshops &&
            Array.isArray(workshops.workshops)
        ) {
            console.warn(
                'API returned workshops wrapped in object, using workshops.workshops'
            )
        ...
        console.warn(
            'Unexpected workshops API response format:',
            typeof workshops,
            workshops
        )
        return { props: { workshops: [] } }
