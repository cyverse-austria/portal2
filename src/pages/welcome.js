...
const useStyles = makeStyles()(theme => ({
    ...
    logo: {
        paddingTop: '10%',
        marginTop: '0',
        [theme.breakpoints.down('md')]: { paddingTop: '10%', marginTop: '0' },
    },
    tagline: {
        [theme.breakpoints.down('md')]: { fontSize: '1.5rem', marginTop: '3%' },
        [theme.breakpoints.down('sm')]: { fontSize: '1.7rem', marginTop: '5%' },
        [theme.breakpoints.down('sm')]: { fontSize: '1.3rem', marginTop: '5%' },
    },
    blueSection: {
        height: '100vh',
        width: '50vw',
...
const Right = props => {
    ...
    if (forgotPassword)
        return (
            <ForgotPassword
                {...props}
                cancelHandler={() => setForgotPassword(false)}
            />
        )

    if (signup) return <SignUp {...props} />
...
const ForgotPassword = ({ startTimeHMAC, cancelHandler }) => {
    ...
    const submitPasswordReset = async email => {
        try {
            ...
            setSubmitting(false)
            setSubmitted(true)
            if (resp !== 'success') setSubmitError(resp)
        } catch (error) {
    ...
    const backButton = (
        <Link component="button" onClick={cancelHandler}>
...
const SignUp = ({ startTimeHMAC, firstNameId, lastNameId }) => {
            ...
            if (value.length >= 3) {
                debounce.current = setTimeout(async () => {
                    ...
                    if (institutions.length == 0)
                        setInstitutionError(
                            'Not found, please try a different search term or enter "other"'
                        )
                    else {
                        if (institutions.length == MAX_RESULTS)
                            institutions.unshift({
                                disabled: true,
                                id: 0,
                                name: `Only first ${MAX_RESULTS} matching results shown, please narrow your search`,
                            })
    ...
    const focusHandler = e => {
        ...
        if (
            e.target.id != 'grid_institution_id' &&
            typeof institutionId === 'undefined'
        )
            setInstitutionError(
                'This field is required, please select a result from the dropdown list'
            )
    }

    const [form, setForm] = useState(
        getForm({ firstNameId, lastNameId, inputHandler, focusHandler })
    )

    const handleSelect = (field, option) => {
        if (field.id == 'country_id')
            setCountryId(option.id) // set region based on country
        else if (field.id == 'grid_institution_id') setInstitutionId(option.id)
    }
    ...
    const validate = async (field, value) => {
        if (field.type == 'username') {
            ...
            if (res && res.username) return 'Username already taken'
        } else if (field.type == 'email') {
            const res = await api.checkEmail(value)
            //TODO add error checking here
            if (res && res.email) return 'Email already assigned to an account'
        }
    ...
    const initialValues = allFields.reduce((acc, f) => {
        acc[f.id.toString()] = ''
        return acc
    }, {})
        ...
        <Box width="60%" className={classes.boxWelcome}>
            {isSubmitted ? (
                <Box p={7}>
                    ...
                    <br />
                    <Typography variant="h5" color="textSecondary">
                        Please click on the confirmation link in the email to
            ...
            },
            {
                ...
                fields: [
                    ...
                    },
                    {
                        ...
                        required: true,
                        options: countryId
                            ? properties.regions.filter(
                                  r => r.country_id == countryId
                              ) // populate based on selected country
                            : [
                                  {
                                      id: 4585,
                                      name: 'Please select a country first ...',
                                      disabled: true,
                                  },
                              ],
                    },
